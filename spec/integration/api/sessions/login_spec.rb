  ...
  it_behaves_like "API over HTTPS with Basic Auth" do
    ...
    context 'missing data' do
      ...
      it 'returns JSON formated error message' do
        expect(json_response['message']).to eq('Record Not Found for given [:email, :password] => ["foo", "password"]')
      end
    end
    ...
    context 'data exists' do
      ...
      context 'invalid password' do
        before { login_call(password: 'foo') }
      ...
      context 'valid password' do
        ...
        it 'returns JSON formatted user' do
          ...
          expect(res['users']['token']).to eq(user[:token])
        end
      end
    end
  end
